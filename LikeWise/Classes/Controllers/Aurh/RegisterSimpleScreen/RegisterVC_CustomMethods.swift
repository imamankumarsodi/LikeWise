//
//  RegisterVC_CustomMethods.swift
//  LikeWise
//
...
extension RegisterVC{
    ...
    internal func initialSetup(){
        ...
        CustomMethods.shared.provideCornerRadiusTo(dotView, dotView.frame.size.height/2, [.layerMinXMinYCorner, .layerMaxXMaxYCorner,.layerMaxXMinYCorner, .layerMinXMaxYCorner])
        dotView.backgroundColor = AppColor.placeholderColor
        
        CustomMethods.shared.setupButton(btnLoginRef, AppColor.placeholderColor,AppColor.clearColor,ConstantFonts.TextFieldFont, ConstantTexts.LoginLT)
        viewLoginUL.backgroundColor = AppColor.clearColor
        
        CustomMethods.shared.setupButton(btnRegisterRef, AppColor.textColor,AppColor.clearColor,ConstantFonts.TextFieldFont, ConstantTexts.RegisterLT)
        viewRegisterUL.backgroundColor = AppColor.placeholderColor
        
        
        CustomMethods.shared.provideShadowAndCornerRadius(viewFullName, viewFullName.frame.size.height/2, AppColor.clearColor, 0, 0, 0, 10, 1, AppColor.placeholderColor)
        txtFullName.font = ConstantFonts.TitleNewFont
        txtFullName.textColor = AppColor.textColor
        txtFullName.placeholder = ConstantTexts.namePH
        txtFullName.keyboardType = .default
        ...
        CustomMethods.shared.provideShadowAndCornerRadius(viewUserName, viewUserName.frame.size.height/2, AppColor.clearColor, 0, 0, 0, 10, 1, AppColor.placeholderColor)
        txtUserName.font = ConstantFonts.TitleNewFont
        ...
        CustomMethods.shared.provideShadowAndCornerRadius(viewPassword, viewPassword.frame.size.height/2, AppColor.clearColor, 0, 0, 0, 10, 1, AppColor.placeholderColor)
        txtPassword.font = ConstantFonts.TitleNewFont
        txtPassword.textColor = AppColor.textColor
        txtPassword.placeholder = ConstantTexts.passwordPH
        txtPassword.keyboardType = .default
        txtPassword.isSecureTextEntry = true
        txtPassword.delegate = self
        
        CustomMethods.shared.setupButton(btnLoginNewRef, AppColor.themeColorDark,AppColor.whiteColor,ConstantFonts.TextFieldFont, ConstantTexts.RegisterLT)
    ...
    internal func setupVAlidations(){
        ...
        if txtPassword.text!.trimmingCharacters(in: .whitespaces) == "" {
            HHelper.showAlert(ConstantTexts.enterPasswordAlert)
            return
        }
        
        if txtPassword.text!.count < 6 {
            HHelper.showAlert(ConstantTexts.enterPasswordLengthAlert)
            return
        }
        ...
        userModel.name = txtFullName.text!
        userModel.password = txtPassword.text!
        userModel.deviceType = ConstantTexts.deviceType
        if let devieToken = UserDefaults.standard.value(forKey: "deviceToken") as? String{
            userModel.deviceToken = devieToken
        }
        
        
        hitCheckEmail_User_Api()
...
extension RegisterVC:UITextFieldDelegate{
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        if textField == txtMail{
            ...
            CustomMethods.shared.showSelectUnselectTextFields(true, [viewMail,viewFullName,viewUserName,viewPassword])
        }else if textField == txtFullName{
            
            CustomMethods.shared.showSelectUnselectTextFields(true, [viewFullName,viewMail,viewUserName,viewPassword])
        }else if textField == txtUserName{
            
            CustomMethods.shared.showSelectUnselectTextFields(true, [viewUserName,viewFullName,viewMail,viewPassword])
        }
        else{
            CustomMethods.shared.showSelectUnselectTextFields(true, [viewPassword,viewMail,viewFullName,viewUserName])
        }
    }
    
    func textFieldDidEndEditing(_ textField: UITextField) {
        CustomMethods.shared.showSelectUnselectTextFields(false, [viewFullName,viewMail,viewUserName,viewPassword])
    }
...
extension RegisterVC{
    ...
    internal func hitCheckEmail_User_Api(){
        ...
        SAuthService().formDataService(profileImageArr, withName: withName, fileName: fileName, mimeType: mimeType, parameters: userModel.toJSON(), serviceName: SAuthApi.signCheckEmailUserName, header: headers).done { (response) in
            CustomMethods.shared.hideLoader(view: self.view,nav: (self.navigationController?.navigationBar)!)
            
            print(response)
            
            if let message = response.message as? String{
                if message == "User not exits"{
